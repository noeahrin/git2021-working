...
public class MemberExample {
	public static void main(String[] args) {
		// Map은 키가 중복이 없음
		// Map<키타입, 값타입>
		// 매개변수로 넣어주는 타입: 제너릭, 제너릭, generic
		// 키: Member의 id
		// 값: Member 객체
		Map<String, Member> members = new HashMap<String, Member>();

		// 1. 객체 추가
		// 새로운 객체 생성하고 map변수의 put 매서드로 추가
		Member hong = new Member("hong", "password123", "홍길동", 20);
		members.put("hong", hong);

		Member smith = new Member("john", "abcd123", "John Smith", 30);
		members.put("john", smith);

		// 같은 키 값 객체를 넣으면 값 객체가 덮어 씌워짐(수정이 일어남)
		Member smith2 = new Member("john1234", "asdf332", "John", 25);
		...
		// 3. 맵 목록 조회
		// 맵변수.keySet() -> 맵의 key 목록을 Set 형식의 자료구조로 반환
		// Set: 중복값이 없는 자료구조
		// {"hong", "john"}
		...
		// 맵 요소가 없는지 체크, true라면 요소가 없음
		// members.isEmpty();

		// 해당 키가 있는지 체크, true라면 해당 키가 있음
