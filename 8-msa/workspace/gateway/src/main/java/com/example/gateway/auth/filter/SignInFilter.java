...
public class SignInFilter implements WebFilter {
	...
	public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain filterChain) {
		...
		if (rootPath.equals("auth") && subPath.equals("signin")) {
			...
			Flux<DataBuffer> result = req.getBody().map(body -> unmashal(body)).flatMap(signInReq -> {
				return db.selectOne(query(where("userId").is(signInReq.getUserId()).and("password")
						.is(Hash.getSha512Hex(signInReq.getPassword()))).limit(1), Login.class);
			}).flatMap(login -> {
				return db.selectOne(query(where("userId").is(login.getUserId())).limit(1), Profile.class);
			}).flatMap(profile -> {
				...
				db.update(Profile.class).matching(query(where("userId").is(profile.getUserId())))
						.apply(update("sessionId", sessionId)).subscribe();
