...
//		System.out.println(rootPath + ">" + subPath);
		...
		if (rootPath.equals("auth") && subPath.equals("signup")) {
			...
			Flux<DataBuffer> result = req.getBody()
					...
					.flatMap(signUpReq -> {
						return db.insert(Login.class).using(Login.builder().userId(signUpReq.getUserId())
								// 패스워드는 단방향 암호화로 만듦
								.password(Hash.getSha512Hex(signUpReq.getPassword())).build())
								.then(db.insert(Profile.class)
										.using(Profile.builder().userId(signUpReq.getUserId())
												.username(signUpReq.getUsername()).email(signUpReq.getEmail())
												.role(signUpReq.getRole()).img(signUpReq.getImg()).build()));
					})
					// 3. 응답 처리
					.flatMap(profile -> {
